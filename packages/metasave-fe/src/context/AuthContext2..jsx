...
export const AuthContextProvider = ({children}) => {
  ...
  const login = async () => {
    ...
    const getCFAddress = async(PRIV_KEY, signer) => {
      ...
      const AAProvider = createSmartAccountClient({
        transport: rpcTransport,
        chain,
        account: new createLightAccount({
          transport: rpcTransport,
          signer,
          chain
        })
  ...
  const checkLoggedIn = async (web3auth) => {
    ...
    if (web3auth?.status === 'connected') {
      ...
      const walletAddress = await walletProvider.getAddress()
      const priv_key = await walletProvider.getPrivateKey()
      const signer = await walletProvider.getSignature()
      console.log('priv_key', priv_key)
